# analytics-query-index-v1 — AURORA Analytical Tables & Query Index Contract

## Purpose

Cross-service analytical contract specifying the ClickHouse query projection tables, column types, snapshot isolation rules, and primary sort keys for database `aurora`.

## Schema Version

`query_index_version: "analytics-query-index-v1"`

## Canonical Source vs Derived Index Relationship

```text
CANONICAL (Immutable Source of Truth)
    MinIO Gold Snapshot + manifest.json + Gold Parquet
           │
           │ (Indexed by python-ml-worker)
           ▼
DERIVED (Rebuildable Analytical Projection)
    ClickHouse Analytical Tables (aurora database)
```

ClickHouse stores rebuildable derived analytical tables. Deleting ClickHouse data does NOT alter canonical Gold dataset truth.
...
### 1. `gold_snapshots_v1` (Snapshot Index Registry)

Derived index metadata table recording indexed snapshot status.
...
### 2. `candidate_features_v1` (Candidate Query Projection)

Query projection of `gold-candidate-v1`.
...
### 3. `anomaly_lightcurve_v1` (Light Curve Anomaly Projection)
...
### 4. `anomaly_tpf_v1` (TPF Anomaly Evidence Projection)
...
### 5. `anomaly_ffi_v1` (FFI Anomaly Evidence Projection)
...
## Mandatory Snapshot Isolation Query Rule

Because Gold snapshots are cumulative across observing sectors, analytical queries MUST filter by `snapshot_id`:

```sql
SELECT count()
FROM aurora.candidate_features_v1
WHERE snapshot_id = 'gold-v1-abc123456789';
```

Executing `SELECT count() FROM candidate_features_v1` without a `snapshot_id` filter is strictly forbidden as it double-counts cumulative snapshots.
