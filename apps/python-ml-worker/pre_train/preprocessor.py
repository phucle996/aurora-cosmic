"""Candidate Preprocessing & Statistical Imputation (Pre-training Stage 1)."""
...
class CandidatePreprocessor:
    ...
    def transform_features(self, rows: List[Dict[str, Any]]) -> np.ndarray:
        """Transform rows to float32 feature matrix using fitted TRAIN stats."""
        if (
    ...
    def from_json(cls, json_str: str) -> "CandidatePreprocessor":
        """Load and reconstruct CandidatePreprocessor from a JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_json_file(cls, path: str) -> "CandidatePreprocessor":
        """Load and reconstruct CandidatePreprocessor from a JSON file."""
