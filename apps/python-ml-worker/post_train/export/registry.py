"""Model Registry, Immutable Packages & Champion Promotion (Post-training Stage 3)."""
...
class ModelRegistry:
    ...
    def register_model_package(
        ...
        model_version = train_data.get("model_version", "candidate-tabular-mlp-v1")
        preprocessing_version = train_data.get(
        ...
        gold_manifest_sha = train_data["gold_manifest_sha256"]
        split_id = train_data["split_id"]
        dataset_view_version = train_data["dataset_view_version"]
        dataset_view_fp = train_data["dataset_view_fingerprint"]
        ...
        if os.path.exists(package_dir) and os.path.exists(manifest_path):
            existing = self.load_model_manifest(manifest_path)
            if existing.model_fingerprint != model_fp:
                raise ModelPackageConflictError(
                    f"CONFLICT: Model package {model_id} already exists with different fingerprint"
                )
            return existing

        os.makedirs(package_dir, exist_ok=True)
        shutil.copy2(model_pt_source_path, os.path.join(package_dir, "model.pt"))
        shutil.copy2(
            preprocessing_json_source_path,
            os.path.join(package_dir, "preprocessing.json"),
        )
        shutil.copy2(
            training_run_manifest_path,
            os.path.join(package_dir, "training_run_manifest.json"),
        )
        shutil.copy2(
            evaluation_run_manifest_path,
            os.path.join(package_dir, "evaluation_run_manifest.json"),
        )
    ...
    def promote_model(
        ...
        """Promote a challenger model package to champion if it outperforms current champion."""
        ...
        if current_champion is None:
            ...
            if task == "candidate_vetting":
                ...
                eval_base = self.evaluation_root or os.path.join(
                    self.registry_root, "evaluations"
                )
    ...
    def rollback_model(
        ...
        """Roll back current champion to target committed model package."""
        package_dir = os.path.join(self._task_dir(task), target_model_id)
        manifest_path = os.path.join(package_dir, "manifest.json")
        if not os.path.exists(manifest_path):
            raise ModelRegistryError(
                f"Target rollback model package does not exist: {target_model_id}"
            )
