...
def test_candidate_challenger_promotion_success_and_rejection():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        registry = ModelRegistry(
            registry_root=os.path.join(tmp_dir, "models"),
            evaluation_root=os.path.join(tmp_dir, "evaluations"),
        )
        import hashlib
        ...
        # Save eval_a metrics in tmp_dir/evaluations/... so promote_model can read champion metrics
        champ_eval_dir = os.path.join(
            tmp_dir, "evaluations", "runs", "candidate", "eval-cand-v1-modelA"
        )
