...
def setup_inference_planning_environment(root_dir: str):
    ...
    runtime_dir = os.path.join(
        root_dir, "models", "runtime", "candidate", "run-cand-pkg-123"
    )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for j in jobs:
            ...
            job_file = os.path.join(
                manifest_dir, "inference-jobs", "candidate", f"{j.job_id}.json"
            )
...
def test_planning_unqualified_runtime_fails():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        unval_pkg = os.path.join(
            tmp_dir, "models", "runtime", "candidate", "run-unqualified-999"
        )
