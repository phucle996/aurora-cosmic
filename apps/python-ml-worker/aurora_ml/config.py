...
class Config:
    def __init__(self):
        ...
        self.lc_feature_version = os.getenv(
            "AURORA_LC_FEATURE_VERSION", "lc-features-v1"
        )
        self.bls_min_period_days = float(
            os.getenv("AURORA_LC_BLS_MIN_PERIOD_DAYS", "0.5")
        )
        self.bls_max_period_days = float(
            os.getenv("AURORA_LC_BLS_MAX_PERIOD_DAYS", "20.0")
        )
        ...
        if self.bls_max_period_days <= self.bls_min_period_days:
            raise ValueError(
                "AURORA_LC_BLS_MAX_PERIOD_DAYS must be > AURORA_LC_BLS_MIN_PERIOD_DAYS"
            )
        ...
        self.tpf_feature_version = os.getenv(
            "AURORA_TPF_FEATURE_VERSION", "tpf-vetting-v1"
        )
        self.tpf_transit_window_factor = float(
            os.getenv("AURORA_TPF_TRANSIT_WINDOW_FACTOR", "1.0")
        )
        self.tpf_out_guard_factor = float(
            os.getenv("AURORA_TPF_OUT_GUARD_FACTOR", "2.0")
        )
        self.tpf_min_in_transit_cadences = int(
            os.getenv("AURORA_TPF_MIN_IN_TRANSIT_CADENCES", "3")
        )
        self.tpf_min_out_transit_cadences = int(
            os.getenv("AURORA_TPF_MIN_OUT_TRANSIT_CADENCES", "20")
        )
        self.ffi_feature_version = os.getenv(
            "AURORA_FFI_FEATURE_VERSION", "ffi-evidence-v1"
        )
        ...
        if self.tpf_out_guard_factor < self.tpf_transit_window_factor:
            raise ValueError(
                "AURORA_TPF_OUT_GUARD_FACTOR must be >= AURORA_TPF_TRANSIT_WINDOW_FACTOR"
            )
        ...
        self.toi_period_rel_tolerance = float(
            os.getenv("AURORA_TOI_PERIOD_REL_TOLERANCE", "0.05")
        )
        self.label_policy_version = os.getenv(
            "AURORA_LABEL_POLICY_VERSION", "candidate-label-policy-v1"
        )
