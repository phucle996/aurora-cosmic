...
def find_latest_gold_snapshot(client: Minio, bucket: str) -> Optional[str]:
    ...
    try:
        objects = list(
            client.list_objects(bucket, prefix="gold/snapshots/", recursive=False)
        )
        snapshot_dirs = [
            obj.object_name.strip("/").split("/")[-1] for obj in objects if obj.is_dir
        ]
        if snapshot_dirs:
...
def generate_synthetic_features(n_samples: int = 120) -> List[Dict[str, Any]]:
    ...
    for i in range(n_samples):
        ...
        depth = float(
            ...
            if is_planet
            else np.random.uniform(0.0001, 0.002)
        )
        rows.append(
            {
...
def run_training_pipeline(payload: Dict[str, Any], config: Config) -> Dict[str, Any]:
    ...
    LOGGER.info(
        "Starting automated training job %s: task=%s epochs=%d lr=%f base_model=%s mode=%s",
        job_id,
        task,
        epochs,
        learning_rate,
    ...
    if isinstance(gold_snapshot_ids, str):
        gold_snapshot_ids = [
            s.strip() for s in gold_snapshot_ids.split(",") if s.strip()
        ]
    ...
    LOGGER.info(
        "Aggregating %d Gold snapshots for single unified training: %s",
    ...
    for snap_id in gold_snapshot_ids:
        ...
        try:
            parquet_obj = minio_client.get_object(
                bucket, f"gold/snapshots/{snap_id}/features.parquet"
            )
        ...
        if snap_loaded == 0:
            try:
                objects = minio_client.list_objects(
                    bucket, prefix=f"gold/snapshots/{snap_id}/data/", recursive=True
                )
                for obj in objects:
                ...
                LOGGER.warning(
                    "Could not list parquet files for snapshot %s: %s", snap_id, exc
                )

        if snap_loaded > 0:
            LOGGER.info(
                "Loaded %d rows from Gold snapshot %s (total accumulated: %d)",
                snap_loaded,
                snap_id,
                len(rows),
            )
    ...
    if not rows:
        LOGGER.warning(
            "No rows loaded from Gold snapshots. Generating rich synthetic astrophysical dataset."
        )
    ...
    with tempfile.TemporaryDirectory(prefix="aurora_train_") as temp_dir:
        ...
        if training_mode != "scratch" and base_model_id:
            ...
            if base_model_id == "champion":
                try:
                    champ_task = (
                        "candidate" if task == "candidate_vetting" else "anomaly"
                    )
                    champ_obj = minio_client.get_object(
                        bucket, f"models/{champ_task}/champion.json"
                    )
                    champ_data = json.loads(champ_obj.read().decode("utf-8"))
                    target_model_id = champ_data.get("model_id") or champ_data.get(
                        "runtime_package_id"
                    )
                except Exception as exc:
                    LOGGER.info(
                        "No active champion pointer found for task %s (%s).", task, exc
                    )
                ...
                for key in possible_keys:
                    try:
                        ...
                        base_model_path = os.path.join(
                            temp_dir, "base_model_weights.pt"
                        )
                        with open(base_model_path, "wb") as f:
                            f.write(obj.read())
                        LOGGER.info(
                            "Successfully loaded base model weights from s3://%s/%s for continual fine-tuning",
                            bucket,
                            key,
                        )
                        break
        ...
        gold_manifest = GoldSnapshotManifest(
            ...
            if snapshot_type == "CANDIDATE"
            else "gold-anomaly-v1",
            feature_versions={
                "lc": "lc-features-v1",
                "tpf": "tpf-vetting-v1",
                "ffi": "ffi-evidence-v1",
        ...
        if task == "candidate_vetting":
            ...
            LOGGER.info(
                "Training PyTorch candidate vetting model on GPU (fine_tune=%s)...",
            ...
            eval_manifest_path = os.path.join(
                candidate_temp_dir, "evaluation_manifest.json"
            )
            ...
            model_package = registry.register_model_package(
                ...
                training_run_manifest_path=os.path.join(
                    candidate_temp_dir, "manifest.json"
                ),
                evaluation_run_manifest_path=eval_manifest_path,
                model_pt_source_path=os.path.join(candidate_temp_dir, "model.pt"),
                preprocessing_json_source_path=os.path.join(
                    candidate_temp_dir, "preprocessing.json"
                ),
            )

            exporter = RuntimeExporter(
                registry_root=registry_dir, runtime_root=runtime_dir
            )
            ...
            eval_manifest_path = os.path.join(
                anomaly_temp_dir, "evaluation_manifest.json"
            )
            with open(eval_manifest_path, "w", encoding="utf-8") as f:
                json.dump(
                    {
            ...
            model_package = registry.register_model_package(
                ...
                training_run_manifest_path=os.path.join(
                    anomaly_temp_dir, "manifest.json"
                ),
                evaluation_run_manifest_path=eval_manifest_path,
                model_pt_source_path=os.path.join(anomaly_temp_dir, "model.pt"),
                preprocessing_json_source_path=os.path.join(
                    anomaly_temp_dir, "preprocessing.json"
                ),
            )
        ...
        s3_prefix = (
            f"models/runtime/{final_task_name}/{final_model_id}/{runtime_pkg_id}"
        )
        ...
        if auto_promote:
            ...
            LOGGER.info(
                "Promoted %s (%s) to champion in s3://%s/%s",
                final_model_id,
                runtime_pkg_id,
                bucket,
                champion_key,
