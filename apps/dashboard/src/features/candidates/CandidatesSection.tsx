...
export default function CandidatesSection({
  ...
  const submitReview = useCallback(async (label: CandidateReview['training_label']): Promise<void> => {
    if (!selected || !snapshotID) return;
    setReviewing(true);
    setReviewMessage(undefined);
    try {
      await apiFetch<{ status: string }>('/v1/models/training-cohort/labels', {
        method: 'POST',
        body: JSON.stringify({
      ...
      setReviewMessage(label === 'POSITIVE' ? 'Đã xác nhận làm positive training evidence.' : label === 'NEGATIVE' ? 'Đã loại và ghi negative training evidence.' : 'Đã giữ lại để theo dõi thêm.');
    } catch (reviewError) {
      setReviewMessage(reviewError instanceof Error ? reviewError.message : 'Không lưu được quyết định review.');
    } finally {
      setReviewing(false);
    }
  }, [loadCandidate, selected, snapshotID]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid min-w-0 gap-6 2xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        ...
        <Card className="min-w-0 overflow-hidden">
          ...
          <CardContent>
            {detailLoading ? <LoadingState /> : !selected || !detail ? <EmptyState label="Chọn một candidate để xem chi tiết." /> : <CandidateDetail detail={detail} detailPath={detailPath} reviewing={reviewing} reviewMessage={reviewMessage} onReview={submitReview} />}
          </CardContent>
        </Card>
      </div>
...
function CandidateDetail({ detail, detailPath, reviewing, reviewMessage, onReview }: { detail: CandidateDetail; detailPath: string; reviewing: boolean; reviewMessage?: string; onReview: (label: CandidateReview['training_label']) => Promise<void> }): JSX.Element {
  const { candidate, evidence, review } = detail;
  return <div className="space-y-4"><div className="rounded-lg border border-border bg-muted/30 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono font-semibold text-primary">TIC {candidate.tic_id}</p><p className="mt-1 text-xs text-muted-foreground">{candidate.model_version}</p></div><Badge variant={candidate.above_threshold ? 'default' : 'outline'}>{candidate.above_threshold ? 'priority review' : 'below threshold'}</Badge></div><Separator className="my-3" /><dl className="grid grid-cols-2 gap-3 text-xs"><InfoItem label="Vetting score" value={formatScore(candidate.candidate_score)} /><InfoItem label="Threshold" value={formatScore(candidate.decision_threshold)} /><InfoItem label="Sector" value={`${candidate.sector}`} /><InfoItem label="Detected" value={formatDate(candidate.predicted_at)} /><InfoItem label="Runtime" value={candidate.runtime_package_id} /><InfoItem label="Validation" value={candidate.runtime_validation_id || '—'} /></dl></div><div className="grid grid-cols-2 gap-2"><EvidenceChip icon={BarChart3} label="BLS" value={evidence.bls_available ? `${formatNumber(evidence.bls_period, 2)} d` : 'not available'} /><EvidenceChip icon={Telescope} label="TOI catalog" value={evidence.matched_toi_id || evidence.toi_match_status || 'unmatched'} /><EvidenceChip icon={Database} label="Samples" value={evidence.n_points.toLocaleString()} /><EvidenceChip icon={Gauge} label="Transit evidence" value={evidence.transit_evidence_available ? 'available' : 'not available'} /></div><div className="rounded-lg border border-primary/25 bg-primary/5 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">Human review decision</p><p className="mt-1 text-xs text-muted-foreground">Stored in the ClickHouse training cohort; immutable Gold evidence is unchanged.</p></div><Badge variant={review?.review_status === 'REVIEWED' ? 'default' : 'outline'}>{review?.training_label || 'UNRESOLVED'}</Badge></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><Button size="sm" disabled={reviewing} onClick={() => void onReview('POSITIVE')}><CheckCircle2 />Confirm signal</Button><Button size="sm" variant="destructive" disabled={reviewing} onClick={() => void onReview('NEGATIVE')}><XCircle />Reject</Button><Button size="sm" variant="outline" disabled={reviewing} onClick={() => void onReview('UNRESOLVED')}><CircleHelp />Needs follow-up</Button></div>{reviewMessage && <p className="mt-3 text-xs text-muted-foreground">{reviewMessage}</p>}{review?.updated_at && <p className="mt-2 font-mono text-[11px] text-muted-foreground">{review.label_source || 'AUTO'} · {formatDate(review.updated_at)}</p>}</div><Button asChild className="w-full"><a href={`${detailPath}/${encodeURIComponent(candidate.prediction_id)}?snapshot_id=${encodeURIComponent(candidate.gold_snapshot_id)}`}>Open physics & habitability detail</a></Button><div className="flex items-center gap-2 text-xs text-muted-foreground"><Database className="size-4" />Feature version {evidence.feature_version || '—'} · lineage verified by snapshot</div></div>;
}
