...
import { ArrowRight, Beaker, Database, LoaderCircle, RefreshCw, Sparkles, Target } from 'lucide-react';
...
const workbenches = [
  ...
  { path: '/research-factory/candidates', title: 'Candidate Review', detail: 'Rank ML candidates, inspect immutable evidence and persist scientific adjudication.', icon: Sparkles },
];
...
export default function ResearchOverviewPage(): JSX.Element {
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      const [goldControl, targets, inference] = await Promise.all([
        apiFetch<GoldControlResponse>('/v1/enrichment/control'),
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">{workbenches.map((item) => { const Icon = item.icon; return <Card key={item.path} className="flex min-w-0 flex-col"><CardHeader><Icon className="size-5 text-primary" /><CardTitle className="mt-3 text-base">{item.title}</CardTitle><CardDescription>{item.detail}</CardDescription></CardHeader><CardContent className="mt-auto"><Button asChild variant="outline" size="sm"><Link to={item.path}>Open <ArrowRight /></Link></Button></CardContent></Card>; })}</div>
    <Card className="border-primary/20 bg-primary/5"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Database className="size-4 text-primary" /> Reproducibility boundary</CardTitle><CardDescription>{snapshot ? <>Current Gold evidence is pinned at <span className="font-mono text-primary">{snapshot}</span>. Research views preserve source limitations instead of filling absent TIC, TPF or BLS fields.</> : 'A reproducible research result begins only after Gold has committed an immutable snapshot.'}</CardDescription></CardHeader></Card>
  </div>;
}
