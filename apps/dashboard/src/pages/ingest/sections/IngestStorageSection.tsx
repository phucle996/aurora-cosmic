...
function statusVariant(status: string): 'default' | 'secondary' | 'destructive' | 'outline' {
  ...
  if (status === 'running' || status === 'downloading' || status === 'draining') return 'secondary';
  return 'outline';
}
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">Theo dõi tiến độ tải dữ liệu và kiểm tra Bronze object cùng một màn hình.</p>
      </div>
    </div>
    ...
    <Card className="rounded-md"><CardHeader className="border-b border-border/60"><div className="flex items-start justify-between gap-3"><div><CardTitle>Ingest controls</CardTitle><CardDescription>Thu nhận sản phẩm MAST vào Bronze với giới hạn an toàn 100 GiB cho mỗi run.</CardDescription></div>{(controlJob || status?.status) && <Badge variant={statusVariant(controlJob?.status ?? status?.status ?? 'not_observed')}>{controlJob?.status ?? status?.status}</Badge>}</div></CardHeader><CardContent className="pt-5"><form className="grid gap-3 md:grid-cols-[1fr_1fr_auto]" onSubmit={startIngest}><label className="space-y-1 text-xs text-muted-foreground">Sector<Input type="number" min={1} value={sector} onChange={(event) => setSector(event.target.value)} /></label><label className="space-y-1 text-xs text-muted-foreground">Download workers<Input type="number" min={1} max={64} value={concurrency} onChange={(event) => setConcurrency(event.target.value)} /></label><div className="flex items-end gap-2">{ingestIsRunning && controlJob?.job_id ? <Button type="button" variant="destructive" onClick={() => void cancelIngest()} disabled={controlBusy || controlJob.status === 'draining'}><Square />{controlJob.status === 'draining' ? 'Draining…' : controlJob.status === 'cancelling' ? 'Cancelling…' : 'Stop ingest'}</Button> : <Button type="submit" disabled={controlBusy || ingestIsRunning}><Play />{ingestIsRunning ? 'Ingest running…' : controlBusy ? 'Starting…' : 'Start ingest'}</Button>}</div></form>{(controlJob?.error || status?.error) && <p className="mt-3 text-xs text-destructive">{controlJob?.error || status?.error}</p>}<p className="mt-3 text-xs text-muted-foreground">{controlJob?.status === 'draining' ? 'Worker đang hoàn tất file hiện tại; không nhận thêm file mới.' : 'Run được checkpoint liên tục để có thể tiếp tục an toàn sau khi gián đoạn.'}</p></CardContent></Card>

    <div className="grid min-w-0 gap-6 2xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      <Card className="min-w-0 h-[620px] rounded-md">
        <CardHeader className="border-b border-border/60"><div className="flex items-start justify-between gap-3"><div><CardTitle>Ingest progress</CardTitle><CardDescription>Tiến độ bền vững của run hiện tại</CardDescription></div><Badge variant={statusVariant(status?.status ?? 'not_observed')}>{status?.status ?? 'not observed'}</Badge></div></CardHeader>
        <CardContent className="h-[540px] space-y-5 overflow-x-hidden overflow-y-auto pt-5">
          {status?.observed ? <>
            ...
            {status.source === 'api-runtime' && status.total_products === 0 && status.status === 'running' && <div className="border border-primary/30 bg-primary/5 p-3 text-xs text-muted-foreground">Run đã được chấp nhận. Đang tìm sản phẩm MAST và chờ kết quả đầu tiên.</div>}
