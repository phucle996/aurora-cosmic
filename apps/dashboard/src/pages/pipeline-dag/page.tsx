...
function goldBuilderStatus(overview?: GoldControlOverview): HopStatus {
  ...
  if (state === 'RUNNING' || state === 'DRAINING' || state === 'CATALOG_SYNCING') return 'running';
...
function hopStatus(value?: string, fallback: HopStatus = 'not_observed'): HopStatus {
  ...
  if (status === 'running' || status === 'draining' || status === 'idle' || status === 'catalog_syncing') return 'running';
  return fallback;
}
...
export default function PipelineDagPage(): JSX.Element {
  ...
  const hops = useMemo<Hop[]>(() => {
    ...
    const pipelineHops: Hop[] = [
      ...
      { id: 'checkpoint', stepNumber: '07', label: 'Crash-Safe Checkpoint Store', shortTitle: 'Durable processing state', description: 'Persists idempotent state only after a Silver artifact has been verified.', astronomyGoal: isHistory ? historicalScope : 'Allow safe resume without duplicate processing.', contract: 'checkpoints/preprocessing/objects/<id>.json', status: upstreamStatus('checkpoint'), input: 'Verified Silver object', output: 'Durable checkpoint', metrics: isHistory ? undefined : metricsByHop.get('checkpoint'), telemetry: isHistory ? undefined : telemetryByHop.get('checkpoint') },
      { id: 'lineage', stepNumber: '08', label: 'Lineage & Provenance Commit', shortTitle: 'Bronze → Silver provenance', description: 'Commits immutable evidence linking source, Bronze, processor version and Silver output.', astronomyGoal: isHistory ? historicalScope : 'Provide an auditable Bronze-to-Silver identity chain.', contract: 'lineage/v1/<lineage-id>.json', status: upstreamStatus('lineage'), input: 'Durable checkpoint', output: 'Committed provenance', metrics: isHistory ? undefined : metricsByHop.get('lineage'), telemetry: isHistory ? undefined : telemetryByHop.get('lineage') },
      { id: 'event', stepNumber: '09', label: 'Silver-Ready Publish', shortTitle: 'Downstream-ready data', description: 'Publishes the verified Silver identity only after checkpoint and lineage commit.', astronomyGoal: isHistory ? historicalScope : 'Release only durable, provenance-complete science artifacts downstream.', contract: 'aurora.v1.silver.<product>.ready', status: upstreamStatus('event'), input: 'Committed provenance', output: 'Silver-ready event', metrics: isHistory ? undefined : metricsByHop.get('event'), telemetry: isHistory ? undefined : telemetryByHop.get('event') },
      { id: 'ack', stepNumber: '10', label: 'Bronze Message ACK', shortTitle: 'Delivery finalized', description: 'Acknowledges the Bronze message only after the Silver-ready event is published.', astronomyGoal: isHistory ? historicalScope : 'Prevent loss or duplicate processing across worker restarts.', contract: 'NATS durable consumer ACK', status: upstreamStatus('ack'), input: 'Published Silver-ready event', output: 'Acknowledged Bronze message', metrics: isHistory ? undefined : metricsByHop.get('ack'), telemetry: isHistory ? undefined : telemetryByHop.get('ack') },
      { id: 'gold-pairing', stepNumber: 'G01', label: 'LC + TPF Pairing & Batch Readiness', shortTitle: 'Research-ready target pairs', description: 'Pairs each Silver light curve with exactly one Target Pixel context before scientific enrichment.', astronomyGoal: historicalRun ? `${historicalInputs.toLocaleString()} targets entered completed Gold batches.` : `${liveReady.toLocaleString()} eligible LC/TPF pairs; ${(readiness?.missing_tpf ?? 0).toLocaleString()} LC still miss TPF evidence.`, contract: 'research-ready-target-pair-v4', status: historicalRun ? evidencedPhaseStatus('gold-pairing', completedEvents('gold-pairing').some((event) => event.input_records > 0) || completedBatchEvidence, runStatus) : liveFineStatus(0), input: 'Silver-ready LC + TPF', output: 'Eligible target pairs', metrics: historicalRun ? componentMetrics('gold-pairing', historicalInputs, historicalOutputs) : { input_records: Math.max(livePending, liveReady + liveTPF), output_rows: liveReady }, telemetry: historicalRun ? componentTelemetry('gold-pairing') : undefined },
      { id: 'gold-catalog', stepNumber: 8, label: 'TIC + TOI Catalog Resolution', shortTitle: 'Verified stellar context', description: 'Resolves immutable TIC stellar parameters and TOI reference evidence for the active target batch.', astronomyGoal: `${(catalog?.target_count ?? historicalInputs).toLocaleString()} targets have catalog resolution evidence in scope.`, contract: 'catalog-enrichment-v4', status: phaseStatus('gold-catalog', 1), input: 'Eligible target pairs', output: 'Verified TIC + TOI context', metrics: historicalRun ? componentMetrics('gold-catalog', historicalInputs, 0) : { input_records: Math.max(catalog?.target_count ?? 0, liveReady), output_rows: liveCatalogRecords }, telemetry: historicalRun ? componentTelemetry('gold-catalog') : undefined },
      { id: 'gold-lc-features', stepNumber: 9, label: 'Light-Curve Statistical Features', shortTitle: 'Cadence and variability evidence', description: 'Computes time coverage, cadence, robust flux distribution, uncertainty and variability summaries.', astronomyGoal: 'Produce deterministic light-curve feature records with a versioned scientific fingerprint.', contract: 'lc-features-v1', status: phaseStatus('gold-lc-features', 2), input: 'Paired Silver light curve', output: 'LC statistical feature rows', metrics: historicalRun ? componentMetrics('gold-lc-features', historicalInputs, historicalOutputs) : { input_records: liveReady, output_rows: 0 }, telemetry: historicalRun ? componentTelemetry('gold-lc-features') : undefined },
