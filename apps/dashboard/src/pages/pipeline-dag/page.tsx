...
const dagConnections: DagConnection[] = [
  ...
  { source: 'gold-lc-features', target: 'gold-tpf-evidence', label: 'Transit ephemeris', lane: 'target-pixel' },
  { source: 'gold-catalog', target: 'gold-candidate', label: 'TIC + TOI context', lane: 'merge' },
  { source: 'gold-lc-features', target: 'gold-candidate', label: 'Features & BLS evidence', lane: 'merge' },
...
export default function PipelineDagPage(): JSX.Element {
  ...
  const hops = useMemo<Hop[]>(() => {
    ...
    const pipelineHops: Hop[] = [
      ...
      { id: 'gold-lc-features', stepNumber: 'G03', label: 'Light Curve Features & BLS Transit Search', shortTitle: 'LC Features & BLS Ephemeris', description: 'Computes 16-dim flux morphology features and runs Box Least Squares (BLS) transit period search.', astronomyGoal: 'Quantifies flux variability and searches for candidate exoplanet transit dips.', contract: '16-dim morphology vector + bls_period, bls_duration, bls_depth, bls_power', status: phaseStatus('gold-lc-features', 2), input: 'Paired normalized flux series', output: 'LC features & BLS candidate ephemeris', metrics: componentMetrics('gold-lc-features', historicalInputs || liveReady, historicalInputs || liveReady), telemetry: componentTelemetry('gold-lc-features') },
      { id: 'gold-tpf-evidence', stepNumber: 'G04', label: 'TPF Centroid Motion & Deficit Vetting', shortTitle: 'Pixel MAD & In-Transit Deficit Centroid', description: 'Measures in-transit flux deficit centroid against stellar position to detect background eclipsing binaries.', astronomyGoal: 'Rejects false positives caused by nearby eclipsing binary contamination.', contract: 'pixel_mad_median, variability_peak_fraction, transit_deficit_sum, center_offset_px', status: phaseStatus('gold-tpf-evidence', 3), input: 'TPF image cube + BLS ephemeris', output: 'TPF spatial vetting vector', metrics: componentMetrics('gold-tpf-evidence', historicalInputs || liveTPF, historicalInputs || liveTPF), telemetry: componentTelemetry('gold-tpf-evidence') },
      { id: 'gold-candidate', stepNumber: 'G05', label: 'Multimodal Candidate Assembly', shortTitle: 'Assembled Candidate Discovery Record', description: 'Combines LC features, BLS ephemeris, TPF spatial evidence, and TIC context into a unified candidate row.', astronomyGoal: historicalRun ? `${historicalOutputs.toLocaleString()} candidate records were assembled for this historical ticket.` : 'Unifies multimodal discovery evidence before serialization.', contract: 'Candidate discovery schema with strict tier-based validation gates', status: phaseStatus('gold-candidate', 4), input: 'LC + BLS + TPF + TIC records', output: 'Candidate Gold record', metrics: componentMetrics('gold-candidate', historicalInputs || livePendingLC, historicalOutputs || livePendingLC), telemetry: componentTelemetry('gold-candidate') },
      { id: 'gold-parquet', stepNumber: 'G06', label: 'Gold Candidate Parquet Materialize', shortTitle: 'Gold Discovery Parquet Object', description: 'Writes Snappy-compressed columnar Parquet files containing full candidate feature rows to object storage.', astronomyGoal: historicalRun ? `${artifactCount} Gold Parquet artifacts were written for this ticket.` : 'Commits immutable Gold data layer artifacts for ML inference and analysis.', contract: 'gold/tess/candidates/snapshot=<id>/sector=<sector>/part-*.parquet', status: phaseStatus('gold-parquet', 5), input: 'Candidate Gold records', output: 'Gold Parquet artifact', metrics: componentMetrics('gold-parquet', historicalOutputs || livePending, historicalOutputs || livePending), telemetry: componentTelemetry('gold-parquet') },
      { id: 'gold-index', stepNumber: 'G07', label: 'ClickHouse Analytical Indexing', shortTitle: 'ReplacingMergeTree Candidate Index', description: 'Inserts candidate rows into candidate_features_v1 and updates gold_snapshots_v1 ledger in ClickHouse.', astronomyGoal: historicalRun ? `${(historicalRun.run.indexed_rows ?? 0).toLocaleString()} candidate rows were indexed into ClickHouse.` : 'Enables low-latency SQL queries and ML inference feature retrieval.', contract: 'aurora.candidate_features_v1 (ReplacingMergeTree)', status: phaseStatus('gold-index', 6), input: 'Gold Parquet artifact', output: 'Indexed ClickHouse rows', metrics: componentMetrics('gold-index', historicalOutputs || livePending, historicalRun?.run.indexed_rows || livePending, historicalRun?.run.indexed_rows || livePending), telemetry: componentTelemetry('gold-index') },
      { id: 'gold-commit', stepNumber: 'G08', label: 'Atomic Snapshot Commit & Lineage', shortTitle: 'Committed Snapshot & Provenance Seal', description: 'Emits atomic manifest pointer to MinIO, seals data lineage ledger, and updates pipeline state.', astronomyGoal: historicalRun ? (historicalRun.run.last_snapshot_id ? `Committed snapshot ${historicalRun.run.last_snapshot_id} as immutable release.` : 'Durable run completed without an active snapshot seal.') : (goldControl?.runtime?.last_snapshot_id ? `Committed snapshot ${goldControl.runtime.last_snapshot_id} as immutable release.` : 'Awaiting complete batch before committing immutable snapshot.'), contract: 'control/enrichment.json pointer + lineage/gold/<snapshot_id>.json', status: goldCommitStatus, input: 'Indexed projection confirmation', output: 'Atomic snapshot commit', metrics: componentMetrics('gold-commit', historicalOutputs || evidenceOutputs, historicalOutputs || evidenceOutputs, historicalRun?.run.indexed_rows || livePending), telemetry: componentTelemetry('gold-commit') },
    ];

    const goldPhaseOrder = ['gold-pairing', 'gold-catalog', 'gold-lc-features', 'gold-tpf-evidence', 'gold-candidate', 'gold-parquet', 'gold-index', 'gold-commit'];
