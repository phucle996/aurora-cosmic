...
export function QualityMaskChart({ metrics, telemetry, modality }: { mode?: 'stream' | 'batch'; metrics?: Record<string, number>; telemetry?: Telemetry; totalFiles?: number; modality?: 'lightcurve' | 'target-pixel' }): JSX.Element {
  ...
  const isBaseline = total === 0 && !countsObserved;
  ...
  return <div className="space-y-3">
    {isBaseline && <div className="flex items-center justify-between border border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground"><span className="flex items-center gap-1.5 font-medium"><span className="size-2 rounded-full bg-amber-500" />Khung phân tích cơ sở: chưa có completion evidence hoặc cadence counters (hiển thị mức nền 0).</span></div>}
