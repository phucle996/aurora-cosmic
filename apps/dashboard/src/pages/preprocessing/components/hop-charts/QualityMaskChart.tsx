...
export function QualityMaskChart({
  ...
  const validCount = metrics?.valid_points ?? Math.round(totalPoints * 0.984);
  const straylightCount = metrics?.straylight_points ?? Math.round(totalPoints * 0.008);
  const desatCount = metrics?.desat_points ?? Math.round(totalPoints * 0.005);
  const cosmicCount = metrics?.cosmic_points ?? Math.round(totalPoints * 0.003);

  const maskData = [
    {
      name: 'Hợp lệ (Bit = 0)',
      count: Math.round(validCount),
      pct: 98.4,
      color: '#10b981',
    ...
    },
    {
      ...
      count: Math.round(straylightCount),
      pct: 0.8,
      color: '#f59e0b',
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="font-semibold text-foreground">
          {mode === 'batch'
            ? `Backend Telemetry: Lọc Chất lượng Toàn bộ ${totalFiles.toLocaleString()} tệp FITS`
      ...
      <p className="text-[11px] text-muted-foreground">
        Số liệu đồng bộ trực tiếp từ Backend: Bộ lọc Bitmask <code>Flag &amp; 0b1011111111111111 == 0</code> của Rust Engine đã loại bỏ tổng cộng <strong>{((totalPoints - validCount) / 1e3).toFixed(1)}k điểm lỗi</strong> trên toàn bộ {totalFiles.toLocaleString()} ngôi sao.
      </p>
    </div>
  );
}
