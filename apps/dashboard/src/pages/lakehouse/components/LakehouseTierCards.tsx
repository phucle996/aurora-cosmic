...
export function LakehouseTierCards({
  ...
  const bronzeUsedPercent = bronzeData
    ? Math.min(100, Math.round(((bronzeData.total_bytes ?? 0) / bronzeBufferCapacity) * 100))
    : 0;
  ...
  return (
    <section aria-label="Lakehouse tier inventory" className="grid gap-px overflow-hidden border border-border/70 bg-border/70 md:grid-cols-3">
      <button type="button" aria-pressed={activeTab === 'bronze'} onClick={() => onTabChange('bronze')} className={`min-w-0 p-4 text-left transition-colors sm:p-5 ${activeTab === 'bronze' ? 'bg-primary/[0.07]' : 'bg-background/80 hover:bg-muted/40'}`}>
        ...
        <div className="mt-1 flex items-baseline justify-between gap-3"><span className="font-mono text-2xl font-semibold tabular-nums">{bronzeData ? (bronzeData.total ?? 0).toLocaleString() : '—'}</span><span className="font-mono text-xs text-muted-foreground">{bronzeData ? formatBytes(bronzeData.total_bytes ?? 0) : 'Click to load'}</span></div>
        <div className="mt-4 space-y-1.5"><div className="flex justify-between font-mono text-[9px] uppercase tracking-[0.08em] text-muted-foreground"><span>100 GiB rolling buffer</span><span>{bronzeData ? `${bronzeUsedPercent}%` : '—'}</span></div><Progress value={bronzeUsedPercent} className="h-1" /></div>
      </button>
      ...
      <button type="button" aria-pressed={activeTab === 'silver'} onClick={() => onTabChange('silver')} className={`min-w-0 p-4 text-left transition-colors sm:p-5 ${activeTab === 'silver' ? 'bg-primary/[0.07]' : 'bg-background/80 hover:bg-muted/40'}`}>
        ...
        <div className="mt-1 flex items-baseline justify-between gap-3"><span className="font-mono text-2xl font-semibold tabular-nums">{silverData ? (silverData.total ?? 0).toLocaleString() : '—'}</span><span className="font-mono text-xs text-muted-foreground">{silverData ? formatBytes(silverData.total_bytes ?? 0) : 'Click to load'}</span></div>
      ...
      <button type="button" aria-pressed={activeTab === 'gold'} onClick={() => onTabChange('gold')} className={`min-w-0 p-4 text-left transition-colors sm:p-5 ${activeTab === 'gold' ? 'bg-primary/[0.07]' : 'bg-background/80 hover:bg-muted/40'}`}>
        ...
        <div className="mt-1 flex items-baseline justify-between gap-3"><span className="font-mono text-2xl font-semibold tabular-nums">{goldData ? (goldData.total ?? 0).toLocaleString() : '—'}</span><span className="font-mono text-xs text-muted-foreground">{goldData ? formatBytes(goldData.total_bytes ?? 0) : 'Click to load'}</span></div>
