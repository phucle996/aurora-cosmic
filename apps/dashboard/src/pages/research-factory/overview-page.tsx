...
import { ArrowRight, Beaker, Database, GitBranch, LoaderCircle, Microscope, Orbit, RefreshCw, SearchCheck, Sparkles, Target } from 'lucide-react';
...
const workbenches = [
  ...
  { path: '/research-factory/workbench', title: 'Observation Workbench', detail: 'Inspect indexed, measured light-curve samples and evidence-backed phase folds.', icon: Microscope },
  { path: '/research-factory/transit-candidates', title: 'Transit Candidates', detail: 'Analyse ML-ranked transit evidence without treating a score as a verdict.', icon: Sparkles },
  { path: '/research-factory/systems', title: '3D Systems & Habitability', detail: 'Visualise only candidates with sufficient real stellar and transit inputs.', icon: Orbit },
  { path: '/research-factory/vetting', title: 'Target Vetting', detail: 'Review the live ML candidate queue alongside its immutable Gold evidence.', icon: SearchCheck },
  { path: '/research-factory/evidence', title: 'Evidence & Runs', detail: 'Trace committed Gold snapshots and their inference provenance.', icon: GitBranch },
];
...
export default function ResearchFactoryOverviewPage(): JSX.Element {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{workbenches.map((item) => { const Icon = item.icon; return <Card key={item.path} className="flex min-w-0 flex-col"><CardHeader><Icon className="size-5 text-primary" /><CardTitle className="mt-3 text-base">{item.title}</CardTitle><CardDescription>{item.detail}</CardDescription></CardHeader><CardContent className="mt-auto"><Button asChild variant="outline" size="sm"><Link to={item.path}>Open <ArrowRight /></Link></Button></CardContent></Card>; })}</div>
