...
// Route page for one persisted candidate and its scientific evidence.
export default function CandidateDetailPage(): JSX.Element {
  const { predictionId = '' } = useParams();
  const location = useLocation();
  const [search] = useSearchParams();
  const queuePath = location.pathname.startsWith('/research-factory/') ? '/research-factory/candidates' : '/candidates';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <Button asChild variant="outline"><Link to={`/research-factory/workbench/${candidate.tic_id}?sector=${candidate.sector}&snapshot_id=${encodeURIComponent(snapshot)}`}><Star />Open host target</Link></Button>
      </div>
