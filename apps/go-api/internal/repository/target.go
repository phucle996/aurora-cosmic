...
ap AS (
    SELECT
        p.tic_id AS tic_id,
        p.sector AS sector,
        p.prediction_id AS anomaly_prediction_id,
        toFloat64(p.reconstruction_mse) AS anomaly_score
...
LEFT JOIN ap ON ap.tic_id = t.tic_id AND ap.sector = t.sector`
	...
	if filter.HasAnomaly != nil {
		...
		conditions = append(conditions, "if(ap.anomaly_prediction_id != '', 1, 0) = ?")
		whereArgs = append(whereArgs, flag)
	}
	if filter.PipelineStatus != "" {
		conditions = append(conditions, "multiIf(ap.anomaly_prediction_id != '' OR cp.candidate_prediction_id != '', 'scored', gf.gold_points > 0, 'ingested', 'discovered') = ?")
	...
	query := ctePrefix + `SELECT
		...
		multiIf(ap.anomaly_prediction_id != '' OR cp.candidate_prediction_id != '', 'scored', gf.gold_points > 0, 'ingested', 'discovered') AS pipeline_status,
...
func (r *TargetClickHouse) GetTarget(ctx context.Context, ticID int64, sector int, snapshotID string) (*entity.TargetDetail, error) {
	sectorFilter := ""
	args := []any{snapshotID, snapshotID, ticID, ticID, ticID, ticID}
	if sector > 0 {
...
SELECT
    ...
    toFloat64(ifNull(cp.candidate_score, 0)) AS candidate_score,
    cast(ifNull(cp.candidate_above_threshold, 0) AS Bool) AS candidate_above_threshold,
    cast(if(ifNull(ap.anomaly_prediction_id, '') != '', 1, 0) AS Bool) AS has_anomaly,
    ifNull(ap.anomaly_prediction_id, '') AS anomaly_prediction_id,
    toFloat64(ifNull(ap.anomaly_score, 0)) AS anomaly_score,
    multiIf(
...
LIMIT 1`
	...
	if err := r.client.Select(ctx, &targets, targetQuery, args...); err != nil {
		return nil, fmt.Errorf("select target detail: %w", err)
	}
	if len(targets) == 0 {
