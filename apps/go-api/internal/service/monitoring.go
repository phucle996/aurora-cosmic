...
// ============================================================================
// DANH SÁCH CÁC COMPONENT ĐƯỢC GIÁM SÁT (Native Prometheus Exporters)
// ============================================================================
var components = []componentSpec{
	...
	},
	{
		...
		Metrics: []metricSpec{
			...
			{Key: "backlog", Name: "Bronze backlog pending", Unit: "messages", Kind: "gauge", Query: "sum(aurora_preprocessor_backlog_pending)"},
	...
	},
	{
		...
		Metrics: []metricSpec{
			...
			{Key: "gpu_utilization", Name: "Training GPU device utilization", Unit: "percent", Kind: "gauge", Query: "max(aurora_ml_gpu_utilization_percent)"},
			{Key: "gpu_memory_used", Name: "Training GPU device memory used", Unit: "bytes", Kind: "gauge", Query: "max(aurora_ml_gpu_memory_used_bytes)"},
			{Key: "gpu_memory_total", Name: "Training GPU device memory total", Unit: "bytes", Kind: "gauge", Query: "max(aurora_ml_gpu_memory_total_bytes)"},
		},
	},
	{
...
// IsValidComponent kiểm tra mã định danh component có hợp lệ không
func IsValidComponent(id string) bool {
	if id == "" || id == "all" {
		return true
	}
...
// ============================================================================
// Query nhận vào cửa sổ thời gian (window) và componentID lọc ("all" hoặc từng component),
// phát động các goroutine truy vấn song song tới Prometheus rồi trả về dữ liệu chuỗi thời gian.
func (s *MonitoringService) Query(ctx context.Context, window entity.MonitoringWindow, componentID string) ([]entity.MonitoringComponent, error) {
	if s.prometheus == nil {
		return nil, fmt.Errorf("Prometheus monitoring is unavailable")
	}

	// 1. Chọn lọc các component theo yêu cầu
	selected, err := selectComponents(componentID)
	if err != nil {
		return nil, err
	}
...
// selectComponents lọc danh sách component dựa trên componentID
func selectComponents(componentID string) ([]componentSpec, error) {
	...
	return nil, fmt.Errorf("unknown monitoring component %q", componentID)
}
