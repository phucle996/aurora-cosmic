...
# flushes from wedging the local filesystem during an Enrichment batch.
...
def _large_cube_lock(scratch_path: Path):
    ...
    try:
        import fcntl
    except ImportError:  # pragma: no cover - Deployed on Linux.
        return nullcontext()
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            for batch in parquet.iter_batches(
                ...
                time_col = batch.column("time")
                try:
                    time[offset:end] = np.asarray(
                        time_col.to_numpy(zero_copy_only=False), dtype=np.float64
                    )
                except Exception:
                    time[offset:end] = _float_values(time_col.to_pylist())

                flux_col = batch.column("flux")
                expected_pixels = dimensions[0] * dimensions[1]
                # High-performance fast path: bulk vector extraction via Arrow inner array
                if (
                    hasattr(flux_col, "values")
                    and flux_col.null_count == 0
                    and len(flux_col.values) == batch_size * expected_pixels
                ):
