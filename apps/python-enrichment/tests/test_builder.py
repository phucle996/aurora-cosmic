...
def test_multimodal_readiness_waits_then_emits_one_complete_gold_unit():
    ...
    pending = [("checkpoints/enrichment/pending/event-1.json", lc_event)]
...
def test_on_demand_catalog_sync_pins_and_reuses_exact_batch_snapshots(monkeypatch):
    ...
    monkeypatch.setattr("pipeline.catalogs.fetch_tic_rows", tic_rows)
    monkeypatch.setattr("pipeline.catalogs.fetch_toi_rows", toi_rows)
...
def test_readiness_selects_the_newest_complete_silver_revision_deterministically():
    ...
    batches, summary = readiness.collect_ready(
        [("checkpoints/enrichment/pending/event-1.json", lightcurve)],
        max_targets=1,
    )
...
def test_gold_operator_control_defaults_to_paused_and_persists_runtime_status():
    ...
    stream = EnrichmentControl(
        mode="STREAM",
        max_batch_records=250,
        idle_flush_seconds=180,
        command_id="gold-1",
    )
    store.put_json("aurora", "control/enrichment.json", stream.to_dict())
    assert load_control(store, "aurora") == stream
    assert (
        store.get_json("aurora", "control/enrichment.json")["max_batch_records"] == 250
    )
    ...
    status = store.get_json("aurora", "control/enrichment/status.json")
...
def test_recovers_unextracted_silver_from_committed_lineage_into_pending_queue():
    ...
    builder = EnrichmentBuilder(store)
...
def test_legacy_partial_gold_does_not_suppress_research_ready_recovery():
    ...
    recovered = EnrichmentBuilder(store).recover_pending_from_lineage("aurora")
    assert len(recovered) == 1
...
def test_gold_builder_rejects_partial_gold_inputs():
    ...
    try:
        EnrichmentBuilder(store).build_candidate([event], set_current=True)
    except EnrichmentBuildError as exc:
...
def test_gold_builder_pins_catalog_snapshots_and_auto_labels_confirmed_toi():
    ...
    feature = EnrichmentBuilder(store)._lightcurve_features(event)
    ...
    result = EnrichmentBuilder(store).build_candidate(
