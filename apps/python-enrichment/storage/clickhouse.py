"""Idempotent Gold/Enrichment-to-ClickHouse query-index projection.

MinIO snapshots and committed manifests are canonical and immutable.
ClickHouse is a derived analytical index owned by the Enrichment service
and can always be re-projected from a committed manifest.
"""
...
from pipeline.training_cohort import COHORT_POLICY_VERSION, label_rows
from storage.object_store import ObjectStore

if TYPE_CHECKING:
    from pipeline.materializer import EnrichmentBuildResult

LOGGER = logging.getLogger("aurora-enrichment-projector")
...
class EnrichmentClickHouseProjector:
    """Projects one committed snapshot, exactly once per manifest hash."""
    ...
    def _ready(self, snapshot_id: str, manifest_sha: str) -> bool:
        """Check if this snapshot has already been projected with identical manifest checksum."""
    ...
    def _ensure_lineage_schema(self) -> None:
        """Ensure the immutable Gold lineage inputs table exists."""
        self.clickhouse.command(
            """CREATE TABLE IF NOT EXISTS gold_lineage_inputs_v1 (
    ...
    def _project_training_cohort(
        ...
        """Project auto-labeled training cohort rows into ClickHouse."""
    ...
    def _candidate_context_from_table(table: pa.Table) -> dict[str, tuple[int, int]]:
        """Extract (tic_id, sector) mapping keyed by source_product_id from an Arrow table."""
    ...
    def project(self, result: EnrichmentBuildResult) -> int:
        """Index a result whose immutable manifest has already been committed."""
        # Ensure target analytical schemas exist
        self._ensure_candidate_schema()
        self._ensure_lightcurve_sample_schema()
        self._ensure_training_cohort_schema()
        self._ensure_lineage_schema()
        ...
        # Skip duplicate projection if already indexed with identical manifest hash
        if self._ready(result.snapshot_id, manifest_sha):
            self.backfill_training_cohort(result.snapshot_id)
            return 0

        # Idempotent cleanup of prior attempt for this snapshot_id
        self.clickhouse.command(
        ...
        # Stream each candidate Parquet table into ClickHouse
        for artifact in manifest.get("artifacts", []):
        ...
        # Record snapshot entry in gold_snapshots_v1
        indexed_at = datetime.now(timezone.utc)
        self.clickhouse.insert(
            "gold_snapshots_v1",
            [
        ...
        # Record lineage inputs
        datasets = [
            str(artifact.get("dataset"))
            for artifact in manifest.get("artifacts", [])
            if artifact.get("dataset")
        ]
        lineage_rows = []
        for inp in manifest.get("inputs", []):
            source_id = str(inp.get("source_product_id") or "").strip()
            silver_key = str(inp.get("silver_object_key") or "").strip()
            if source_id or silver_key:
                lineage_rows.append(
                    [
                        source_id,
                        silver_key,
        ...
        # Record projection metadata document in MinIO
        self.store.put_json(
            self.bucket,
