...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        worker_concurrency = int(
            os.getenv(
                "AURORA_ENRICHMENT_WORKER_CONCURRENCY",
                os.getenv("AURORA_GOLD_WORKER_CONCURRENCY", "2"),
            )
        )
        if worker_concurrency < 1:
            raise ValueError("AURORA_ENRICHMENT_WORKER_CONCURRENCY must be positive")
        ...
        return cls(
            ...
            scratch_dir=os.getenv(
                "AURORA_ENRICHMENT_SCRATCH_DIR",
                os.getenv("AURORA_GOLD_SCRATCH_DIR", ".runtime/enrichment-scratch"),
            ),
            durable=os.getenv(
                "AURORA_ENRICHMENT_DURABLE",
                os.getenv("AURORA_GOLD_DURABLE", "aurora-enrichment"),
            ),
            stream=os.getenv(
                "AURORA_ENRICHMENT_STREAM",
                os.getenv("AURORA_GOLD_STREAM", "AURORA_SILVER"),
            ),
            metrics_addr=os.getenv(
                "AURORA_ENRICHMENT_METRICS_ADDR",
                os.getenv("AURORA_GOLD_METRICS_ADDR", ":8088"),
            ),
        )
