...
-- Target Pixel File (TPF) spatial evidence & postage stamp samples
CREATE TABLE IF NOT EXISTS aurora.tpf_samples_v1 (
    tic_id Int64,
    sector Int32,
    source_product_id String,
    rows Int32 DEFAULT 11,
    cols Int32 DEFAULT 11,
    aperture_mask Array(UInt8),
    median_flux_map Array(Float64),
    difference_flux_map Array(Float64),
    centroid_row Float64,
    centroid_col Float64,
    centroid_offset_pixels Float64,
    pixel_mad_median Float64,
    variability_peak_fraction Float64,
    projected_at DateTime64(3, 'UTC') DEFAULT now64()
)
ENGINE = ReplacingMergeTree(projected_at)
PARTITION BY sector
ORDER BY (sector, tic_id);
